...
isInvalid | boolean | Bootstrap 4 only. Adds the `is-invalid` classname to the `form-control`.
isLoading | boolean | false | Indicate whether an asynchronous data fetch is happening.
isValid | boolean | Bootstrap 4 only. Adds the `is-valid` classname to the `form-control`.
